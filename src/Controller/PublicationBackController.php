...
class PublicationBackController extends AbstractController
{
    ...
    #[Route('/{id}', name: 'app_publication_back_delete2', methods: ['POST'])]
    public function delete2(SessionInterface $session, Request $request, Publication $publication, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete' . $publication->getId(), $request->request->get('_token'))) {
            $entityManager->remove($publication);
            $entityManager->flush();
